...
if (location.protocol === 'https:') {
    socketurl = "wss://" + window.location.hostname + ":8000/wshandler/";
} else {
    socketurl = "ws://" + window.location.hostname + ":8000/wshandler/";
}
...
$(function () {
        ...
        function connectsocket(stype) {
            ...
            ws.onmessage = function (evt) {
                ...
                $('#tx_0 li:first a').attr('href', '/tx/' + msg.t0.txhash);
                $('#txhash0').text(msg.t0.txhash);
                $('#from0').text(msg.t0.from);
                $('#tx_0 li:nth-child(2) a').attr('href', '/address/' + msg.t0.from);
                $('#to0').text(msg.t0.to);
                $('#tx_0 li:nth-child(3) a').attr('href', '/address/' + msg.t0.to);
                $('#time0').text(msg.t0['time']);

                $('#tx_1 li:first a').attr('href', '/tx/' + msg.t1.txhash);
                $('#txhash1').text(msg.t1.txhash);
                $('#from1').text(msg.t1.from);
                $('#tx_1 li:nth-child(2) a').attr('href', '/address/' + msg.t1.from);
                $('#to1').text(msg.t1.to);
                $('#tx_1 li:nth-child(3) a').attr('href', '/address/' + msg.t1.to);
                $('#time1').text(msg.t1['time']);

                $('#tx_2 li:nth-child(1) a').attr('href', '/tx/' + msg.t2.txhash);
                $('#txhash2').text(msg.t2.txhash);
                $('#tx_2 li:nth-child(2) a').attr('href', '/address/' + msg.t2.from);
                $('#from2').text(msg.t2.from);
                $('#tx_2 li:nth-child(3) a').attr('href', '/address/' + msg.t2.to);
                $('#to2').text(msg.t2.to);
                $('#time2').text(msg.t2['time']);

                $('#tx_3 li:first a').attr('href', '/tx/' + msg.t3.txhash);
                $('#txhash3').text(msg.t3.txhash);
                $('#tx_3 li:nth-child(2) a').attr('href', '/address/' + msg.t3.from);
                $('#from3').text(msg.t3.from);
                $('#tx_3 li:nth-child(3) a').attr('href', '/address/' + msg.t3.to);
                $('#to3').text(msg.t3.to);
                $('#time3').text(msg.t3['time']);

                $('#tx_4 li:first a').attr('href', '/tx/' + msg.t4.txhash);
                $('#txhash4').text(msg.t4.txhash);
                $('#from4').text(msg.t4.from);
                $('#tx_4 li:nth-child(2) a').attr('href', '/address/' + msg.t4.from);
                $('#to4').text(msg.t4.to);
                $('#tx_4 li:nth-child(3) a').attr('href', '/address/' + msg.t4.to);
